...
Data-availability caveat (important): the original framework references two
TAIFEX derivatives indicators — 外資台指期未平倉淨空單 and 選擇權 Put/Call
Ratio — that this project does not fetch yet (see fetcher.py's
fetch_tw_chip_indicators() docstring for why). Every condition that depends on
one of those is marked completeness='partial': it's evaluated using only the
leg of the original AND-condition that real data is available for, never
silently dropped or faked. Callers/UI must not treat a partial condition's
'triggered' status the same as a fully-verified one.
...
def _top_conditions(history: List[Dict]) -> List[Dict]:
    ...
    # T3. 外資期現貨雙空 — 只驗證得到「現貨」那半，期貨淨空單無資料
    foreign_series = _series(history, "foreign_net")
    if len(foreign_series) >= FOREIGN_SELL_STREAK_DAYS:
        recent = foreign_series[-FOREIGN_SELL_STREAK_DAYS:]
        spot_sell_streak = all(v < 0 for v in recent)
        status = spot_sell_streak  # partial: futures leg unverifiable
        detail = (f"外資現貨連續{FOREIGN_SELL_STREAK_DAYS}日賣超："
                  f"{'是' if spot_sell_streak else '否'}（{[round(v,1) for v in recent]}億元）。"
                  f"⚠️ 台指期淨空單資料尚未串接，此條件僅驗證現貨那一半")
    else:
        status, detail = None, "歷史資料不足，尚無法判斷"
    conditions.append(_condition("foreign_dual_short", "外資期現貨雙空 (僅現貨可驗證)", status, "partial", detail))

    # T4. 籌碼過度樂觀/散戶接刀 — 只驗證外資賣超+融資不退，P/C Ratio無資料
    margin_amt_series = _series(history, "margin_balance_amount")
    if foreign_series and len(margin_amt_series) >= MARGIN_NOT_RETREATING_LOOKBACK_DAYS + 1:
        ...
        status = foreign_selling_today and margin_not_retreating
        detail = (f"今日外資{'賣超' if foreign_selling_today else '買超'}；"
                  ...
                  f"⚠️ 選擇權 Put/Call Ratio 資料尚未串接，此條件僅驗證外資+融資那一半")
    else:
        status, detail = None, "歷史資料不足，尚無法判斷"
    conditions.append(_condition("retail_holding_bag", "籌碼過度樂觀/散戶接刀 (僅外資+融資可驗證)", status, "partial", detail))
...
def _bottom_conditions(history: List[Dict]) -> List[Dict]:
    ...
    # B3. 外資空單回補與匯率止穩 — 只驗證匯率止穩+現貨轉買，期貨空單回補無資料
    usdtwd_series = _series(history, "usdtwd")
    foreign_series = _series(history, "foreign_net")
    if len(usdtwd_series) >= TWD_DEPRECIATION_LOOKBACK_DAYS + 1 and len(foreign_series) >= 2:
        ...
        status = fx_stabilizing and foreign_flow_reversed
        detail = (f"USD/TWD {TWD_DEPRECIATION_LOOKBACK_DAYS}日變動 {fx_change:+.3f}"
                  ...
                  f"⚠️ 台指期空單回補資料尚未串接，此條件僅驗證匯率+現貨那一半")
    else:
        status, detail = None, "歷史資料不足，尚無法判斷"
    conditions.append(_condition("foreign_short_covering", "外資空單回補與匯率止穩 (僅匯率+現貨可驗證)", status, "partial", detail))
