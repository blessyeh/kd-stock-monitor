...
### ✅ Signal Quantitative-Rigor Upgrades (2026-08-14)
The four items the second architecture review (2026-08-09) flagged, in its own suggested order, are now done — summarized here since they change how the signal panels compute and display, not just add new ones.

- **Graduated 0-100 sub-scores replace hard booleans**: every T1-T5/B1-B3 condition in `signal_confluence.py` now also carries a `score` (0-100, piecewise-linear interpolation via the new `_graduated()` helper, not discrete buckets — so 0.14pp and 0.15pp score nearly identically instead of jumping) alongside its unchanged boolean `status`. `signal_score.py`'s dimension point-awards were rewired to use the same graduated scales rather than flat all-or-nothing bumps. B3 additionally gets a dedicated `bottom_reversal_score()` — a worked 30/35/35-weighted "Bottom Reversal Score" (FX stabilization / foreign flow reversal / futures covering) — shown as its own breakdown under the Signal Confluence panel's bottom group.
- **Signal History + Backtest** (`src/signal_backtest.py`, new module): every run now appends today's signal state (which conditions triggered, both 0-100 scores, TSMC buy-points) to `data/signal_history.json` — one entry per calendar date, same overwrite-on-rerun pattern as `macro_history.json` — then recomputes forward 1D/3D/5D/10D/20D returns, win rate, average return, expectancy, worst-case return, and a Sharpe-like ratio per signal type into `data/signal_backtest.json`, shown in a new "訊號回測：勝率與期望值" dashboard panel. ⚠️ This can only accumulate **forward** from 2026-08-14 — there was no way to reconstruct what these modules would have said on past dates from the raw values already in `macro_history.json`, so every stat reports "樣本不足" (insufficient sample) until enough real occurrences accumulate, the same honest gate used elsewhere in this project.
- **Flow normalization**: the per-stock institutional-flow alert filter (`alert_checker.py`) no longer uses a flat 500,000-share threshold — it's now a ratio of 3-day cumulative foreign net flow against the stock's own 20-day average trading volume (`FOREIGN_FLOW_VOLUME_RATIO_THRESHOLD = 0.3`), so a mega-cap and a small-cap aren't held to the same absolute bar. Chosen over a Z-score against the stock's own institutional-flow history because that history isn't persisted anywhere yet — a Z-score would need weeks of new data collection before producing anything, whereas volume is already available on every stock's existing price history.
- **Data freshness + source-specific scheduling**: `main.py` now caches TW EOD sources (chip flow, per-stock institutional flow, TSMC fundamentals) in `data/.tw_eod_cache.json` and skips re-fetching them on same-calendar-day hourly reruns — those sources are published once per TW trading day, so re-fetching them up to 24x/day was pure waste. US stock prices and macro indicators stay unconditional every run. `summary.json` now carries a `data_freshness` block (per-source `as_of_date` + whether this run actually re-fetched), and the dashboard shows a 🟢/🟡/🔴 freshness badge next to the date text on the Signal Confluence, Signal Score, chip-flow, and TSMC panels — TW EOD sources compare against the last weekday-aware trading day (so a Monday-morning read of Friday's data still shows fresh), not raw calendar "today".

### 🚧 Roadmap | 尚待開發
What's still open, in priority order:
1. **CPI/PCE, Non-Farm Payrolls & Unemployment Rate | CPI/PCE、非農就業與失業率**: Fetchable for free via FRED's no-key CSV endpoint, but these are monthly releases (not continuous prices) — they need a different "latest value + release date + countdown" UI treatment rather than the hourly ticker-card style, so they're deferred rather than shoehorned in.
   *   可透過 FRED 免金鑰 CSV 端點免費取得，但這些是月度公布數據（非連續價格），需要另一種「最新公布值+公布日期+倒數」的呈現方式，而非現有的每小時卡片樣式，因此先不做。
2. **ISM Manufacturing PMI**: Not planned — ISM revoked FRED's license to redistribute this data in 2016, and there's no reliable free official source anymore (real-time PMI is generally paywalled via Trading Economics/Bloomberg).
   *   不在計畫內——ISM 已於 2016 年收回 FRED 的資料授權，目前沒有可靠的官方免費來源（即時 PMI 多半來自付費資料商）。
3. **Git repository housekeeping | Git 倉庫維護**: `docs/data/` was added to `.gitignore` (it's a deploy-time build artifact and shouldn't be version-controlled), but the already-tracked copy still needs a one-time `git rm -r --cached docs/data` to actually stop tracking it.
   *   `docs/data/` 已加入 `.gitignore`（這是部署時才產生的建置產物，不該進版控），但既有已追蹤的檔案還需要手動跑一次 `git rm -r --cached docs/data` 才會真正停止追蹤。
4. **TSMC earnings-call transcript → structured extraction**: `investor.tsmc.com` also publishes each quarter's earnings-conference transcript PDF for free, which contains richer color (customer/platform revenue mix commentary, capex rationale, AI/HPC demand commentary) than the numeric Guidance table alone. An LLM-based structured-extraction pass over the transcript (a fixed JSON schema: revenue by platform, key qualitative guidance callouts, analyst Q&A themes) was floated as a strong follow-on. Not started.
...
### ✅ 訊號量化嚴謹度升級（2026-08-14）
第二次架構審查（2026-08-09）依建議順序列出的四個項目現已完成——以下摘要說明，因為這些變更改動的是各訊號面板的計算與呈現方式本身，不只是新增面板。

- **漸進式 0-100 子分數取代硬性布林判斷**：`signal_confluence.py` 中每一個 T1-T5/B1-B3 條件現在除了原本不變的布林值 `status` 外，也同時攜帶一個 `score`（0-100，透過新增的 `_graduated()` 輔助函式做分段線性內插，而非離散區間——因此 0.14pp 與 0.15pp 的分數幾乎相同，不會跳躍）。`signal_score.py` 各構面的加分邏輯也已改用同一套漸進量表，取代原本非黑即白的加分方式。B3 額外新增專屬的 `bottom_reversal_score()`——一個 30/35/35 加權的「底部反轉分數」（匯率止穩／外資轉強／期貨回補），以獨立分解區塊顯示在「訊號共振」面板底部條件群組下方。
- **訊號事後績效追蹤 + 回測**（`src/signal_backtest.py`，新模組）：每次執行現在都會把當天的訊號狀態（各條件是否觸發、兩個 0-100 分數、台積電買點設定）append 進 `data/signal_history.json`——每個日曆日一筆，覆寫規則與 `macro_history.json` 相同——再重新計算每種訊號在 1D/3D/5D/10D/20D 後的報酬率、勝率、平均報酬、期望值、最差單次報酬與類 Sharpe 比率，寫入 `data/signal_backtest.json`，顯示於新增的「訊號回測：勝率與期望值」儀表板面板。⚠️ 這項功能只能從 2026-08-14 起**向後**累積——`macro_history.json` 裡只有原始指標數值，沒有辦法回推這些模組在過去某一天實際上會判定什麼結果，因此在累積足夠真實樣本前，所有統計數字都會顯示「樣本不足」，與本專案其他地方使用的誠實提示邏輯一致。
- **籌碼流量標準化**：個股法人流向警示濾網（`alert_checker.py`）不再使用固定 500,000 股門檻——改為以 3 日累計外資淨買賣超對個股自身 20 日均量的比率（`FOREIGN_FLOW_VOLUME_RATIO_THRESHOLD = 0.3`）判斷，讓大型權值股與小型股不再套用同一個絕對門檻。選擇比率而非對個股自身法人流向歷史做 Z-score，是因為那段歷史目前完全沒有被保存下來——Z-score 需要數週的新資料累積才有意義，而成交量資料本來就已經存在於每支股票既有的價格歷史中。
- **資料時效 + 依資料源分別排程**：`main.py` 現在會把台股 EOD 資料源（籌碼面、個股法人流向、台積電基本面）快取到 `data/.tw_eod_cache.json`，同一日曆日內的每小時重跑會直接重用快取、不再重新抓取——這些資料本來就只有每個交易日公布一次，過去每小時重抓最多一天浪費近 24 次請求。美股價格與宏觀指標則維持每次執行都重新抓取。`summary.json` 新增 `data_freshness` 區塊（記錄每個資料源的 `as_of_date` 與本次是否真的重新抓取），儀表板也在「訊號共振」「訊號評分」「籌碼面」「台積電」四個面板的日期文字旁加上 🟢/🟡/🔴 資料新鮮度標示——台股 EOD 資料源比對的是「最近一個有考慮週末的交易日」，而非單純的日曆「今天」，因此週一早上顯示週五的資料仍會是綠燈，不會誤判為過期。

### 🚧 尚待開發
以下是目前仍待開發的項目，依優先度排序：
1. **CPI/PCE、非農就業與失業率**：可透過 FRED 免金鑰 CSV 端點免費取得，但這些是月度公布數據（非連續價格），需要另一種「最新公布值+公布日期+倒數」的呈現方式，而非現有的每小時卡片樣式，因此先不做。
2. **ISM 製造業 PMI**：不在計畫內——ISM 已於 2016 年收回 FRED 的資料授權，目前沒有可靠的官方免費來源（即時 PMI 多半來自付費資料商）。
3. **Git 倉庫維護**：`docs/data/` 已加入 `.gitignore`（這是部署時才產生的建置產物，不該進版控），但既有已追蹤的檔案還需要手動跑一次 `git rm -r --cached docs/data` 才會真正停止追蹤。
4. **台積電法說會逐字稿 → 結構化擷取**：`investor.tsmc.com` 也免費公布每季法說會逐字稿PDF，內容比純數字財測指引表更豐富（客戶/平台營收結構評論、資本支出理由、AI/HPC需求評論等）。用LLM對逐字稿做結構化擷取（固定JSON schema：分平台營收、關鍵定性財測描述、法人問答主題）是一個被提出的強力延伸方向。尚未開始。
5. **自建「台積電市場預期指數」**：作為真正市場共識（Consensus，見上方財測指引說明，仍不在範圍內）的長期替代方案，將台積電自身財測＋月營收趨勢＋供應鏈代理指標（NVDA/AVGO/AMD/ASML）＋SOX＋TSM ADR方向整合成單一「市場對台積電的隱含預期正在上升或下降」綜合指數。優先度較低，依專案擁有者自己的排序明確列為延後項目。尚未開始。
6. **分析師市場共識API（例如Financial Modeling Prep）——延後**：確實存在真正的Consensus資料源，但因可靠性/授權/時間點完整性考量而刻意降低優先度——見上方財測指引說明中提到的未來函數偏誤風險（對歷史某一季查詢「今天的」共識，其實已經包含當時市場還不知道的後續資訊，這比完全沒有共識資料更危險，尤其用於回測時）。若真要做，需要明確處理「時間點快照」而非天真地「抓最新」。優先度最低，依專案擁有者自己的排序列為最後一項。
